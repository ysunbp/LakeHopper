                ...
                if mode=='trust':
                    if softmax_score >= confidence:
                        ...
                        if response.split(',')[0] == 'Yes' or response.split('.')[0] == 'Yes':
                            ...
                            llm_decision = 3
                            reasoning = response
                    csv_writer.writerow([table_id, inverted_dict[int(labels[0][col_idx])], labels[0][col_idx], cur_label, cur_label_idx, gt_decision, llm_decision, reasoning, cur_col, i])
...
def LLM_transfer_clustering(model, dataset, current_exploration_round, query_size, sample_size, csv_data_path, label_dict, round, sampled_data_path):
   ...
   csv_path = '../gpt-logs/sudo-v2s-' + str(round)+'.csv'
...
def LLM_evaluate(model, iter, model_save_path, is_test=False, cur_best_loss=100):
    ...
    print('validation/testing time', end-start)
...
def transfer_training_process(hp, csv_data_path, validation_path, test_path, label_dict, model, dataset, exploring_rounds, total_finetune_epochs, previous_dataloaders = []):
    ...
    sampled_data_path = '../sampled_data/sudo-v2s/'
    ...
    if hp.design == 3: 
        ...
        for round in range(exploring_rounds): ###
            ...
            if cur_best_val_loss < cur_best:
                ...
                no_improve = 0
                torch.save(model.state_dict(), '../checkpoints/sudo-v2s-cur_best.pkl')
            else:
                no_improve += 1
            
            if no_improve >= early_stop:
                print('The LLM transfer knowledge is used up! Shift to fine-tuning mode.')
                torch.save(model.state_dict(), '../checkpoints/sudo-v2s-stop-at-'+str(round)+'-'+'.pkl')
                break
        finetune_flag = True
        if finetune_flag:
            model.load_state_dict(torch.load('../checkpoints/sudo-v2s-cur_best.pkl'))  ###
        ...
        model_path = '../checkpoints/sudo-v2s-cur_best.pkl'
...
if __name__ == '__main__':
    ...
    parser.add_argument("--save_path", type=str, default='../checkpoints/sudo-v2s.pkl')
    ...
    semtab_label_path = '../data/Semtab2019/semtab_labels.json'
    model, label_dict = adjust_label_diff(sato_label_path, semtab_label_path, pretrained_model_path='../checkpoints/sudo-v2s-base.pkl')
    ...
    transfer_finetune_dataset = SupAnnDatasetIndex(semtab_training_path, lm='bert', size=56, max_length = 128)
    ...
    if not os.path.exists('../checkpoints/sudo-v2s-base-semtab-finetuned.pkl'):
        print('Start initial fine-tuning on Semtab')
        
        model.load_state_dict(torch.load('../checkpoints/sudo-v2s-after_contrast_transfer_prior_sato.pkl'))
        optimizer = AdamW(model.parameters(), lr=hp.lr)
        model, _ = LLM_finetuning(model, transfer_training_dataloader, valid_iter, hp.save_path, optimizer, hp.n_epochs)
        LLM_evaluate(model, test_iter, hp.save_path, is_test=True, cur_best_loss=100)
        torch.save(model.state_dict(),'../checkpoints/sudo-v2s-base-semtab-finetuned.pkl')
    else:
        hp.n_classes = len(list(label_dict.keys()))
        model = base_model(hp, device=device, lm=hp.lm) # reload the model on the target data lake
