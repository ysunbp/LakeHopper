...
if __name__ == '__main__':
    ...
    for round in rounds:
        if True:
            train_loaders = []
            val_loader = get_loader(path='LakeHopper/data/RECA/'+str(MAX_LEN)+'-lake/fold_'+str(0), batch_size=BS, is_train=False)
            train_loaders.append(get_loader(path='LakeHopper/data/RECA/'+str(MAX_LEN)+'-lake/fold_'+str(1), batch_size=BS, is_train=True))
            test_loader = get_loader(path='LakeHopper/data/RECA/'+str(MAX_LEN)+'-lake/fold_'+str(2), batch_size=BS, is_train=False)
            for lr in lrs:
                print('start training fold', round+1, 'learning rate', lr, 'batch size', BS, 'max length', MAX_LEN)
                model = KREL().cuda()
    
                
                public_bi_path = 'LakeHopper/checkpoints/webtables-RECA-publicbi'+"_lr=1e-5_bs="+str(BS)+'_max='+str(MAX_LEN)
                model_save_path = 'LakeHopper/checkpoints/out_model_p2v
                if not os.path.exists(public_bi_path):
                    ...
                    print('Loading pretrained model on PublicBI')
                model.load_state_dict(torch.load(public_bi_path))
                train_model(model, train_loaders, val_loader, test_loader, lr, model_save_path=model_save_path, size=108)
