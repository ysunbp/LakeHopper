...
def get_annotation(model, annotation_dataset, label_dict, round, cur_round_idx, out_path, confidence=0.9, mode='trust'):
    ...
    with open(out_path, 'a', newline='') as file:
        ...
        csv_writer.writerow(['table_id','col_idx','ground_truth','gt_id','prediction','pred_id','should be','llm_decision','reasoning','data','dataset_tab_idx']) 
...
def LLM_transfer_clustering(model, dataset, current_exploration_round, query_size, sample_size, csv_data_path, label_dict, round, sampled_data_path):
    ...
    This function is called in rounds by the upper level function, it identify the tables to ask for annotation from model and GPT.
    ...
    csv_path = '../gpt-logs/doduo-p2v-' + str(round)+'.csv'
...
def LLM_finetuning(model, dataset_iter, valid_iter, model_save_path, optimizer, epochs, cur_best=100, previous_dataloaders=[], finetune_flag=False):
    ...
    for epoch in range(epochs):
        ...
        if not finetune_flag:
            ...
            cur_best_model = model
    print('trainign time', np.mean(training_times), training_times)
...
def transfer_training_process(hp, csv_data_path, validation_path, test_path, label_dict, model, dataset, exploring_rounds, total_finetune_epochs, previous_dataloaders = []):
    ...
    sampled_data_path = '../sampled_data/doduo-p2v/'
    ...
    if hp.design == 3: 
        ...
        for round in range(exploring_rounds):
            ...
            if cur_best_val_loss < cur_best:
                ...
                no_improve = 0
                torch.save(model.state_dict(), '../checkpoints/doduo-p2v-cur_best.pkl')
            else:
                no_improve += 1
            
            if no_improve >= early_stop:
                print('The LLM transfer knowledge is used up! Shift to fine-tuning mode.')
                torch.save(model.state_dict(), '../checkpoints/doduo-p2v-stop-at-'+str(round)+'-'+'.pkl')
                break
        ...
        current_training_set = SupAnnDatasetIndex(csv_data_path, lm='bert', size=None, max_length = 128, size_column=True) # modify the size according to the budget
        current_dataset_iter = data.DataLoader(dataset=current_training_set,
                                                batch_size=8,
                                                shuffle=False,
                                                num_workers=0,
                                                collate_fn=current_training_set.pad)
        for i in range(38): # modify the ranfe based on the exact number of rounds the model runs
            with open(sampled_data_path+'round-'+str(i)+'-selected.pkl', 'rb') as file:
                selected_data_iter = pickle.load(file)
            previous_dataloaders.append(selected_data_iter)
        previous_dataloaders.append(current_dataset_iter)
        model_path = '../checkpoints/doduo-p2v-cur_best.pkl' # This is for DODUO
...
if __name__ == '__main__':
    ...
    parser.add_argument("--save_path", type=str, default='../checkpoints/doduo-p2v.pkl')
    ...
    LLM_finetuning(model, training_dataset_iter, valid_iter, hp.save_path, optimizer, hp.n_epochs)
