...
import torch
import numpy as np
from dataset_reca import SupAnnDataset, SupAnnDatasetIndex
from dataset_reca_sato import SupAnnDatasetIndexSato
                ...
                if mode=='trust':
                    if softmax_score >= confidence:
                        ...
                        if response.split(',')[0] == 'Yes' or response.split('.')[0] == 'Yes':
                            ...
                            llm_decision = 3
                            reasoning = response
                    csv_writer.writerow([table_id, inverted_dict[int(label_list[0][col_idx])], label_list[0][col_idx], cur_label, cur_label_idx, gt_decision, llm_decision, reasoning, cur_col, i])
...
def LLM_transfer_clustering(model, dataset, current_exploration_round, query_size, sample_size, csv_data_path, label_dict, round, sampled_data_path):
    ...
    csv_path = '../gpt-logs/reca-v2s-' + str(round)+'.csv'
...
def LLM_finetuning(model, dataset_iter, valid_iter, model_save_path, optimizer, epochs, cur_best=100, previous_dataloaders=[], finetune_flag=False):
    ...
    for epoch in range(epochs):
        ...
        if not finetune_flag:
            ...
            cur_best_model = model
    print('average training time', np.mean(training_times), training_times)
...
def LLM_evaluate(model, iter, model_save_path, is_test=False, cur_best_loss=100):
    ...
    print('valid/test time', end-start)
...
def transfer_training_process(hp, csv_data_path, validation_path, test_path, label_dict, model, dataset, exploring_rounds, total_finetune_epochs, previous_dataloaders = []):
    ...
    sampled_data_path = '../sampled_data/reca-v2s/'
    validation_set = SupAnnDatasetIndex(validation_path, max_length = 128, pickle_path = '../data/RECA/semtab-reca-valid.pkl')
    ...
    test_set = SupAnnDatasetIndex(test_path, max_length = 128, pickle_path = '../data/RECA/semtab-reca-test.pkl')
    ...
    if hp.design == 3:
        ...
        optimizer = torch.optim.AdamW(optimizer_grouped_parameters, lr=hp.w_lr)
        decayRate = 1.0
        ...
        for round in range(exploring_rounds): ###
            ...
            selected_dataset = SupAnnDatasetIndex(csv_data_path, size=None, max_length = 128, cur_selected_indices=overall_identified_col_idx, pickle_path = '/export/data/ysunbp/LLM-veri/data/reca-pickle/semtab-reca-train.pkl')
            ...
            if cur_best_val_loss < cur_best:
                ...
                no_improve = 0
                torch.save(model.state_dict(), '../checkpoints/reca-v2s-cur_best.pkl') # in case azure problem, for continue training
            else:
                no_improve += 1
            
            if no_improve >= early_stop:
                print('The LLM transfer knowledge is used up! Shift to fine-tuning mode.')
                torch.save(model.state_dict(), '../checkpoints/reca-v2s-stop-at-'+str(round)+'-'+'.pkl')
                break
        finetune_flag = True

        if finetune_flag:
            model.load_state_dict(torch.load('../checkpoints/reca-v2s-cur_best.pkl'))  ###
            full_set = SupAnnDatasetIndex(csv_data_path, max_length = 128, pickle_path = '../data/RECA/semtab-reca-train.pkl')
        ...
        current_training_set = SupAnnDatasetIndex(csv_data_path, size=None, max_length = 128, size_column=True, pickle_path = '/export/data/ysunbp/LLM-veri/data/reca-pickle/semtab-reca-train.pkl')
        ...
        model_path = '../checkpoints/reca-v2s-cur_best.pkl'
...
def adjust_label_diff(source_label_path, target_label_path, pretrained_model_path):
    ...
    for s,t in source2target_mapping:
        current_model_dict[list(pretrained_model_dict.keys())[-9]][t] = pretrained_model_dict[list(pretrained_model_dict.keys())[-9]][s]
        current_model_dict[list(pretrained_model_dict.keys())[-8]][t] = pretrained_model_dict[list(pretrained_model_dict.keys())[-8]][s]
        current_model_dict[list(pretrained_model_dict.keys())[-7]][t] = pretrained_model_dict[list(pretrained_model_dict.keys())[-7]][s]
        current_model_dict[list(pretrained_model_dict.keys())[-6]][t] = pretrained_model_dict[list(pretrained_model_dict.keys())[-6]][s]
        current_model_dict[list(pretrained_model_dict.keys())[-5]][t] = pretrained_model_dict[list(pretrained_model_dict.keys())[-5]][s]
        current_model_dict[list(pretrained_model_dict.keys())[-4]][t] = pretrained_model_dict[list(pretrained_model_dict.keys())[-4]][s]
        current_model_dict[list(pretrained_model_dict.keys())[-3]] = pretrained_model_dict[list(pretrained_model_dict.keys())[-3]]
        current_model_dict[list(pretrained_model_dict.keys())[-2]] = pretrained_model_dict[list(pretrained_model_dict.keys())[-2]]
        current_model_dict[list(pretrained_model_dict.keys())[-1]] = pretrained_model_dict[list(pretrained_model_dict.keys())[-1]]
...
if __name__ == '__main__':
    setup_seed(1998)
    type_set = 'Here are the set of semantic types we consider: [Film, Lake, Language, Company, Person, VideoGame, City, Currency, Mountain, Scientist, Bird, Plant, TelevisionShow, Animal, Country, AdministrativeRegion, Genre, Newspaper, Airport, AcademicJournal, PopulatedPlace, Wrestler, PoliticalParty, Cricketer, Eukaryote, Saint, Writer, Museum, BaseballPlayer, EducationalInstitution, GovernmentType, SportsTeam, ChristianBishop, Settlement, Royalty, EthnicGroup, University, Diocese, Province, ClubMoss, GridironFootballPlayer, CollegeCoach, Colour, Comedian, Manga, Publisher, Magazine, ComicsCharacter, ComicsCreator, Airline, ConcentrationCamp, Congressman, Region, Conifer, SoccerClub, Criminal, Crustacean, Curler, Cycad, Cyclist, DartsPlayer, Disease, Drug, Economist, Organisation, Embryology, Engineer, Entomologist, Mammal, Mollusca, FashionDesigner, Fern, FictionalCharacter, FigureSkater, Wikidata:Q11424, MusicalArtist, Album, Single, FilmFestival, Fish, Food, RacingDriver, FormulaOneRacer, GrandPrix, Fungus, GaelicGamesPlayer, Town, Village, MusicGenre, GolfPlayer, GovernmentAgency, Governor, OfficeHolder, FormulaOneTeam, AmericanFootballPlayer, AmericanFootballTeam, Band, Guitarist, Instrument, Gymnast, AdultActor, AmateurBoxer, Ambassador, Amphibian, AnatomicalStructure, Artery, Muscle, Nerve, RaceHorse, Anime, Arachnid, BritishRoyalty, Architect, Building, Vein, ArtificialSatellite, School, Astronaut, Athlete, AustralianRulesFootballPlayer, Ship, AutomobileEngine, BadmintonPlayer, Bank, Baronet, BaseballTeam, BaseballLeague, BasketballPlayer, BasketballTeam, BeachVolleyballPlayer, BeautyQueen, Protein, Bodybuilder, River, Bone, Book, Artist, Boxer, Brain, Bridge, TelevisionStation, Infrastructure, BroadcastNetwork, BusinessPerson, Canal, Canoeist, Cardinal, Castle, Planet, Chancellor, Chef, ChessPlayer, ClassicalMusicArtist, HandballPlayer, Historian, HockeyTeam, IceHockeyPlayer, HollywoodCartoon, HorseRider, HorseTrainer, Hospital, Hotel, Insect, Jockey, Journalist, Judge, LacrossePlayer, LawFirm, Legislature, Library, Ligament, Island, Lymph, MartialArtist, Mayor, Automobile, Medician, MemberOfParliament, MilitaryConflict, MilitaryPerson, MilitaryUnit, MilitaryStructure, Aircraft, PersonFunction, Model, Monarch, Moss, MotorcycleRider, SpeedwayRider, MountainRange, MountainPass, Murderer, Song, Musical, NascarDriver, NationalCollegiateAthleticAssociationAthlete, NetballPlayer, Noble, Non-ProfitOrganisation, OlympicEvent, Swimmer, OlympicResult, TennisPlayer, Painter, Park, Philosopher, Photographer, Play, PlayboyPlaymate, Poem, Poet, PokerPlayer, Ideology, President, Politician, Pope, PowerStation, RadioHost, Place, PrimeMinister, Prison, ProtectedArea, PublicTransitSystem, RadioProgram, RadioStation, RailwayLine, Station, RouteOfTransportation, RailwayStation, Tunnel, RailwayTunnel, Religious, Reptile, Restaurant, RoadTunnel, Rocket, Rower, HistoricBuilding, ReligiousBuilding, RugbyPlayer, RugbyClub, ScreenWriter, Sea, Senator, ShoppingMall, SiteOfSpecialScientificInterest, Skater, SkiArea, Skier, SnookerChamp, SnookerPlayer, SoccerLeague, SoccerManager, SoccerPlayer, Software, TennisTournament, SquashPlayer, SumoWrestler, TableTennisPlayer, TelevisionEpisode, TelevisionHost, Theatre, Valley, VoiceActor, Volcano, VolleyballCoach, VolleyballPlayer, Weapon, WineRegion, Grape, WrestlingEvent, YearInSpaceflight, Cleric, CyclingTeam, HistoricPlace] \n'
    ...
    parser.add_argument("--query_size", type=int, default=int(25*2.5))
   ...
   parser.add_argument("--save_path", type=str, default='../checkpoints/reca-v2s.pkl')
    ...
    semtab_label_path = '../data/Semtab2019/semtab_labels.json'
    with open(semtab_label_path, 'r', encoding='utf-8') as file:
        label_dict = json.load(file)

    csv_data_path = '../data/RECA/1.jsonl'
    validation_path = '../data/RECA/0.jsonl'
    ...
    if not os.path.exists('../checkpoints/base-reca-v2s.pkl'):
        ...
        sato_test_dataset = SupAnnDatasetIndexSato(test_path, size=500, max_length = hp.max_len, pickle_path = '/export/data/ysunbp/LLM-veri/data/reca-pickle/K2.pkl')
        
        training_dataset_iter = data.DataLoader(dataset=sato_train_dataset,
                                            batch_size=8,
                                            shuffle=True,
                                            num_workers=0,
                                            collate_fn=sato_train_dataset.pad)
        
        valid_iter = data.DataLoader(dataset=sato_valid_dataset,
                                            batch_size=1,
                                            shuffle=False,
                                            num_workers=0,
                                            collate_fn=sato_valid_dataset.pad)
        
        test_iter = data.DataLoader(dataset=sato_test_dataset,
                                            batch_size=1,
                                            shuffle=False,
                                            num_workers=0,
                                            collate_fn=sato_test_dataset.pad)
        ...
        torch.save(model.state_dict(), '../checkpoints/base-reca-v2s.pkl')
    

    
    if not os.path.exists('../checkpoints/base-reca-v2s-adj.pkl'):
        ### Adjust the last layer
        sato_label_path = '../data/VizNet/label.json'
        semtab_label_path = '../data/Semtab2019/semtab_labels.json'
        model, label_dict = adjust_label_diff(sato_label_path, semtab_label_path, pretrained_model_path='../checkpoints/base-reca-v2s.pkl')
    else:
        model.load_state_dict(torch.load('../checkpoints/base-reca-v2s-adj.pkl'))
    
    
    torch.save(model.state_dict(),hp.save_path)
    semtab_testing_path = '../data/RECA/test.jsonl'
    testing_set = SupAnnDatasetIndex(semtab_testing_path, max_length = 128, pickle_path = '../data/RECA/semtab-reca-test.pkl')
    ...
    semtab_training_path = '../data/RECA/train.jsonl'
    semtab_validation_path = '../data/RECA/valid.jsonl'
    semtab_testing_path = '../data/RECA/test.jsonl'
    transfer_finetune_dataset = SupAnnDatasetIndex(semtab_training_path, size=None, max_length = 128, pickle_path = '../data/RECA/semtab-reca-train.pkl')
    ...
    validation_set = SupAnnDatasetIndex(semtab_validation_path, max_length = 128, pickle_path = '../data/RECA/semtab-reca-valid.pkl')
    ...
    test_iter = data.DataLoader(dataset=testing_set,
                                        ...
                                        num_workers=0,
                                        collate_fn=testing_set.pad)
    if not os.path.exists('../checkpoints/base-reca-v2s-finetuned.pkl'):
        ...
        LLM_evaluate(model, test_iter, hp.save_path, is_test=True, cur_best_loss=100)
        torch.save(model.state_dict(),'../checkpoints/base-reca-v2s-finetuned.pkl')
    else:
        model.load_state_dict(torch.load('../checkpoints/base-reca-v2s-finetuned.pkl'))
    
    print('Storing model at: ', hp.save_path)
    torch.save(model.state_dict(), hp.save_path)
    annotation_dataset = SupAnnDataset(semtab_training_path, semtab_label_path, size=None, max_length = hp.max_len, pickle_path = '../data/RECA/semtab-reca-train.pkl') # used for sample queries from GPT
