...
import torch
import numpy as np
from dataset import SupAnnDataset, SupAnnDatasetIndex
    ...
    with open(out_path, 'a', newline='') as file:
        ...
        for i, item in enumerate(tqdm(annotation_iter)):
            ...
            for col_idx in range(table_width):
                ...
                if cur_label == inverted_dict[int(labels[0][col_idx])]:
                    gt_decision = True
                else:
                    gt_decision = False
                ...
                if mode=='trust':
                    if softmax_score >= confidence:
                        ...
                        if response.split(',')[0] == 'Yes' or response.split('.')[0] == 'Yes':
                            ...
                            llm_decision = 3
                            reasoning = response
                    csv_writer.writerow([table_id, col_idx, inverted_dict[int(labels[0][col_idx])], labels[0][col_idx], cur_label, cur_label_idx, gt_decision, llm_decision, reasoning, cur_col, i])
...
def LLM_transfer_clustering(model, dataset, current_exploration_round, query_size, sample_size, csv_data_path, label_dict, round, sampled_data_path):
    ...
    csv_path = '../gpt-logs/doduo-v2s-' + str(round)+'.csv'
...
def transfer_training_process(hp, csv_data_path, validation_path, test_path, label_dict, model, dataset, exploring_rounds, total_finetune_epochs, previous_dataloaders = []):
    ...
    sampled_data_path = '../sampled_data/doduo-v2s/'
    validation_set = SupAnnDatasetIndex(validation_path, lm='bert', max_length = 128)
    ...
    test_set = SupAnnDatasetIndex(test_path, lm='bert', max_length = 128)
    ...
    if hp.design == 3:
        ...
        for round in range(exploring_rounds): ###
            ...
            if cur_best_val_loss < cur_best:
                ...
                no_improve = 0
                torch.save(model.state_dict(), '../checkpoints/doduo-v2s-cur_best.pkl')
            else:
                no_improve += 1
            
            if no_improve >= early_stop:
                print('The LLM transfer knowledge is used up! Shift to fine-tuning mode.')
                torch.save(model.state_dict(), '../checkpoints/doduo-v2s-stop-at-'+str(round)+'-'+'.pkl')
                break
        finetune_flag = True
        if finetune_flag:
            model.load_state_dict(torch.load('../checkpoints/doduo-v2s-cur_best.pkl'))  ###
        ...
        for i in range(47): # modify the range based on the exact number of rounds the model runs
            with open(sampled_data_path+'round-'+str(i)+'-selected.pkl', 'rb') as file:
                selected_data_iter = pickle.load(file)
            previous_dataloaders.append(selected_data_iter)
        previous_dataloaders.append(current_dataset_iter)
        model_path = '../checkpoints/doduo-v2s-cur_best.pkl' # This is for DODUO
...
if __name__ == '__main__':
    ...
    cur_design = 3 # change to 2 and modify the rounds if you want to start at the point the model terminates from the iterations
    ...
    parser.add_argument("--query_size", type=int, default=25)
    ...
    parser.add_argument("--save_path", type=str, default='../checkpoints/doduo-v2s.pkl')
    ...
    if not os.path.exists('../checkpoints/doduo-v2s-base.pkl'):
        ...
        sato_test_dataset = SupAnnDatasetIndex(test_path, lm=hp.lm, size=500, max_length = hp.max_len)
        ...
        LLM_evaluate(model, test_iter, hp.save_path, is_test=True, cur_best_loss=100)
        torch.save(model.state_dict(), '../checkpoints/doduo-v2s-base.pkl')
    
    if not os.path.exists('../checkpoints/doduo-v2s-base-finetuned.pkl'):
        ### Adjust the last layer
        sato_label_path = '../data/VizNet/label.json'
        semtab_label_path = '../data/Semtab2019/semtab_labels.json'
        model, label_dict = adjust_label_diff(sato_label_path, semtab_label_path, pretrained_model_path='../checkpoints/doduo-v2s-base.pkl')
    else:
        hp.n_classes = len(list(label_dict.keys()))
        model = base_model(hp, device=device, lm=hp.lm)    
    ...
    semtab_validation_path = '../data/Semtab2019/semtab_val_0.csv'
    semtab_testing_path = '../data/Semtab2019/semtab_test_0.csv'
    transfer_finetune_dataset = SupAnnDatasetIndex(semtab_training_path, lm='bert', size=25, max_length = 128)
    ...
    validation_set = SupAnnDatasetIndex(semtab_validation_path, lm='bert', max_length = 128)
    ...
    if not os.path.exists('../checkpoints/doduo-v2s-base-finetuned.pkl'):
        ...
        LLM_evaluate(model, test_iter, hp.save_path, is_test=True, cur_best_loss=100)
        torch.save(model.state_dict(), '../checkpoints/doduo-v2s-base-finetuned.pkl')
    else:
        hp.n_classes = len(list(label_dict.keys()))
        model = base_model(hp, device=device, lm=hp.lm) # reload the model on the target data lake
        model.load_state_dict(torch.load('../checkpoints/doduo-v2s-base-finetuned.pkl'))
